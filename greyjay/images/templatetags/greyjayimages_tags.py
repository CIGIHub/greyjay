...
def image(parser, token):
    ...
    if is_valid:
        return GreyjayImageNode(image_expr, '|'.join(filter_specs), attrs=attrs, output_var_name=output_var_name)  # One line with a change
    else:
        raise template.TemplateSyntaxError(
...
class GreyjayImageNode(ImageNode):
    def render(self, context):
        debug = getattr(settings, 'DEBUG', False)
        try:
            return super(GreyjayImageNode, self).render(context)
        except Exception:
            if debug:
                raise
            else:
